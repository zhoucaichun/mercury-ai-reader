...
大家好，我把这次开源课程大作业的项目范围、协作规则和任务池整理好了。
...
所以我们后续会用 GitHub 管理项目。每个人加入仓库，认领自己的 Issue，用自己的分支开发，通过 Pull Request 合并。README 里会写清楚每个人分工。
...
每位同学认领任务后，可以先把下面通用提示词发给 AI，再附上自己任务对应的专用提示词。
...
| 编号 | 模块                                     | 负责人 | 主要产出                                                |
| ---- | ---------------------------------------- | ------ | ------------------------------------------------------- |
| T0   | 组长 / AI 产品负责人 / 项目留痕          | 组长   | 产品范围、GitHub 管理、PRD、README、AGENTS、LLM 用量统计需求定义、最终验收    |
| T1   | 项目骨架 / 跨平台开发环境 / 基础页面框架 | 待认领 | 项目初始化、目录结构、跨平台运行脚本、基础页面          |
| T2   | 数据模型 / 本地存储                      | 待认领 | Feed、Article、Content、AITaskRun、LLMUsageEvent 等模型 |
| T3   | Feed 解析 / Feed URL 添加                | 待认领 | RSS / Atom 解析和标准化文章数据                         |
| T4   | OPML 导入 / 订阅源管理                   | 待认领 | OPML 解析、订阅源列表、订阅源管理                       |
| T5   | Sync / 文章同步 / 入库                   | 待认领 | 刷新订阅源、去重、入库、同步状态                        |
| T6   | Reader Pipeline / 内容清洗               | 待认领 | source HTML、cleaned HTML、canonical Markdown           |
| T7   | 阅读器 UI / 内容呈现 / 阅读样式          | 待认领 | 文章列表、阅读器、阅读设置、摘要/翻译入口               |
| T8   | Agent Runtime / Prompt Templates         | 待认领 | Agent 状态机、Prompt 模板、错误状态、共用调用契约       |
| T9   | LLM Providers / 模型配置 / 用量统计展示   | 待认领 | 标准 API 配置、Provider / Model 统一调用接口、LLM Usage 统计面板 |
| T10  | Summary Agent                            | 待认领 | 摘要生成、摘要展示、摘要保存                            |
| T11  | Translation Agent / 单篇 Markdown 导出   | 待认领 | 翻译生成、译文展示、单篇导出                            |
...
大家认领后可以马上让 AI 分析自己的任务，不用等待其他同学。
