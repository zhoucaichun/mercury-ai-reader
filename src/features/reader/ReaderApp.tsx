...
const TRANS_PROVIDER_STORAGE_KEY = 'mercury-reader-llm-translation';

function loadTransProviderConfig(): { baseUrl: string; model: string; apiKey: string } | null {
  try {
    const raw = localStorage.getItem(TRANS_PROVIDER_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (parsed.baseUrl && parsed.model && parsed.apiKey) return parsed;
    return null;
  } catch {
    return null;
  }
...
function saveTransProviderConfig(input: { baseUrl: string; model: string; apiKey: string }) {
  const current = loadTransProviderConfig();
  const config = {
    baseUrl: input.baseUrl.trim(),
    model: input.model.trim(),
    apiKey: input.apiKey.trim() || current?.apiKey || ''
  };
  localStorage.setItem(TRANS_PROVIDER_STORAGE_KEY, JSON.stringify(config));
  return config;
}
...
export function ReaderApp() {
  ...
  const [translationProgress, setTranslationProgress] = useState<{ total: number; completed: number }>({ total: 0, completed: 0 });
  ...
  // Feature 3: Separate translation provider state
  const [transProviderBaseUrl, setTransProviderBaseUrl] = useState('');
  const [transProviderModel, setTransProviderModel] = useState('');
  const [transProviderApiKey, setTransProviderApiKey] = useState('');
  ...
  useEffect(() => {
    ...
    setTranslatedTitle('');
    setTranslationProgress({ total: 0, completed: 0 });
  ...
  useEffect(() => {
    ...
    // Load translation provider config
    const transConfig = loadTransProviderConfig();
    if (transConfig) {
      setTransProviderBaseUrl(transConfig.baseUrl);
      setTransProviderModel(transConfig.model);
    }
  }, []);
  ...
  async function handleTranslateArticle(regenerate = false) {
    ...
    try {
      ...
      setTranslationProgress({ total, completed: 0 });
      ...
      if (agentUiPort.translateText && selectedArticle.title) {
        ...
        setTranslationProgress({ total, completed: 1 });
      } else {
        setTranslationProgress({ total, completed: 1 });
      }
      ...
      for (let i = 0; i < translatableBlocks.length; i++) {
        ...
        setTranslationProgress({ total, completed: i + 2 }); // +2: 1 for title + (i+1) paragraphs
      }
  ...
  function ensureProviderConfigured(agentType: 'summary' | 'translation') {
    if (agentType === 'translation') {
      // Check translation provider first, fall back to summary provider
      if (loadTransProviderConfig() || loadReaderLLMProviderConfig()) {
        return true;
      }
    } else {
  ...
  async function handleSaveTransProviderConfig() {
    setTransProviderStatus('saving');
    setTransProviderMessage('');

    try {
      const config = saveTransProviderConfig({
        baseUrl: transProviderBaseUrl,
        model: transProviderModel,
        apiKey: transProviderApiKey
      });
      setTransProviderBaseUrl(config.baseUrl);
      setTransProviderModel(config.model);
      setTransProviderApiKey('');
      setTransProviderStatus('succeeded');
      setTransProviderMessage(copy.providerSaved);
    } catch (error) {
      setTransProviderStatus('failed');
      setTransProviderMessage(error instanceof Error ? error.message : copy.providerTestFailed);
    }
  }

  // Provider status: separate for summary and translation
  const [transProviderStatus, setTransProviderStatus] = useState<ProviderStatus>('idle');
  const [transProviderMessage, setTransProviderMessage] = useState('');
  ...
  async function handleTestTransProviderConnection() {
    setTransProviderStatus('testing');
    setTransProviderMessage('');

    try {
      if (transProviderBaseUrl.trim() && transProviderModel.trim()) {
        const config = saveTransProviderConfig({
          baseUrl: transProviderBaseUrl,
          model: transProviderModel,
          apiKey: transProviderApiKey
      ...
      if (!transConfig) {
        setTransProviderStatus('failed');
        setTransProviderMessage(copy.providerTestFailed);
        return;
      }

      // Create a temporary port with translation provider config to test
      const origConfig = loadReaderLLMProviderConfig();
      saveReaderLLMProviderConfig({ baseUrl: transConfig.baseUrl, model: transConfig.model, apiKey: transConfig.apiKey });
      const port = createBrowserWeek3AgentUiPort();
      const result = await port.testConnection?.();
      // Restore original config
      if (origConfig) {
        saveReaderLLMProviderConfig(origConfig);
      }

      if (result?.status === 'succeeded') {
        setTransProviderStatus('succeeded');
        setTransProviderMessage(`${copy.providerTestSucceeded} ${result.latencyMs ?? 0}ms`);
      } else {
        setTransProviderStatus('failed');
        setTransProviderMessage(result?.errorMessage || copy.providerTestFailed);
      }
    } catch (error) {
      setTransProviderStatus('failed');
      setTransProviderMessage(error instanceof Error ? error.message : copy.providerTestFailed);
    ...
    <main className={shellClassName} style={gridStyle}>
        ...
        {!isInspectorCollapsed && (activePanel === 'summary' || activePanel === 'translation') ? (
          <div className="inspector-section">
            ...
            <div className="agent-status-list">
              ...
              {activePanel === 'translation' && translationProgress.total > 0 ? (
                <div className="translation-progress-info">
                  {translationStatus === 'running'
                    ? `${uiLanguage === 'zh' ? '翻译中' : 'Translating'} ${translationProgress.completed}/${translationProgress.total}`
                    : `${translationProgress.completed}/${translationProgress.total} ${uiLanguage === 'zh' ? '段已翻译' : 'paragraphs translated'}`
                  }
      ...
      {activeDialog === 'settings' ? (
        ...
        >
          ...
          {/* Feature 3: Translation Model Provider Section */}
          <section className="settings-section" aria-label={copy.translationModel}>
            <div className="settings-section-header">
              <Languages size={18} aria-hidden="true" />
              <div>
                <h3>{copy.translationModel}</h3>
                <p>{loadTransProviderConfig() ? copy.providerConfigured : copy.providerMissing}</p>
              </div>
            </div>
            <label className="setting-group">
              <span className="setting-label">{copy.providerBaseUrl}</span>
              <input
                ...
                placeholder={copy.providerBaseUrlPlaceholder}
                type="url"
                value={transProviderBaseUrl}
                onChange={(event) => setTransProviderBaseUrl(event.target.value)}
              />
            </label>
            <label className="setting-group">
              <span className="setting-label">{copy.providerModel}</span>
              <input
                ...
                placeholder={copy.providerModelPlaceholder}
                type="text"
                value={transProviderModel}
                onChange={(event) => setTransProviderModel(event.target.value)}
              />
            </label>
            <label className="setting-group">
              <span className="setting-label">{copy.providerApiKey}</span>
              <input
                ...
                placeholder={copy.providerApiKeyPlaceholder}
                type="password"
                value={transProviderApiKey}
                onChange={(event) => setTransProviderApiKey(event.target.value)}
              />
            </label>
            <p className="settings-note">{copy.providerApiKeyHint}</p>
            <div className="settings-actions">
              <button
                className="tool-button"
                type="button"
                disabled={transProviderStatus === 'saving' || transProviderStatus === 'testing'}
                onClick={() => void handleSaveTransProviderConfig()}
              >
                <Settings size={16} aria-hidden="true" />
                {transProviderStatus === 'saving' ? copy.saving : copy.saveProvider}
              </button>
              <button
                className="primary-button is-compact"
                type="button"
                disabled={transProviderStatus === 'saving' || transProviderStatus === 'testing'}
                onClick={() => void handleTestTransProviderConnection()}
              >
                {transProviderStatus === 'testing' ? <RefreshCw className="spin-icon" size={16} aria-hidden="true" /> : <Wifi size={16} aria-hidden="true" />}
                {copy.providerTest}
              </button>
            </div>
            {transProviderMessage ? (
              <p className={transProviderStatus === 'failed' ? 'agent-error' : 'settings-success'}>{transProviderMessage}</p>
            ) : null}
          </section>
        </DialogShell>
      ) : null}
