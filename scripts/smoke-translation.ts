...
//
// Tests translateArticle() with a mock article's canonicalMarkdown.
// Uses MockLLMProvider (no real API key needed).
// Every call produces a usage event visible in the output.
...
async function main() {
  ...
  // ── Test article (simulates a real article's canonicalMarkdown) ──
  const testArticle = {
  ...
  // ── Test 1: direct Provider path (createMockTranslationAgent) ──────
  ...
  if (result1.result) {
    ...
    console.log(`tokens: ${result1.result.promptTokens ?? "?"} prompt + ${result1.result.completionTokens ?? "?"} completion = ${result1.result.totalTokens ?? "?"} total`);
  ...
  // ── Test 2: translateArticle() without runtime (fallback path) ────
  ...
  try {
    ...
    console.log(`tokens: ${result2.promptTokens ?? "?"} prompt + ${result2.completionTokens ?? "?"} completion`);
  ...
  // ── Test 3: failure path (mock-fail trigger) ──────────────────────
  console.log("--- Test 3: Failure path ---");
  const failArticle = { ...testArticle, canonicalMarkdown: "[mock-fail] This triggers mock failure" };
