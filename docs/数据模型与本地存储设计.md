# Mercury AI Reader — 数据模型与本地存储设计

> 版本：v1.1（最终版）  
> 负责人：T2 林杨  
> 参考：mercury (macOS/Swift/GRDB) 数据模型  
...
本文档沿用 mercury 源码的命名习惯，与任务文档中的中文术语对应如下：

| 本文档术语 | 任务文档术语 | 说明 |
|-----------|-------------|------|
| `Entry` | Article（文章） | Feed 中的一篇文章 |
| `Content` | ArticleContent（文章内容） | 文章的三层内容 |
| `Feed` | 订阅源 | RSS/Atom Feed |
| `SyncLog` | 同步状态/同步记录 | Feed 同步的执行日志 |
| `AgentTaskRun` | AI 任务运行 | 一次 AI 操作的作业记录 |
| `LLMUsageEvent` | LLM 用量事件 | 一次 LLM API 请求的用量记录 |
